...
As hosted, these correspond to the layout of §5.2:
...
https://cdn.example.net/revocations.dedi.json      ← may live on any host the publisher controls
```

`revocations` deliberately sits on a different host to show that the manifest's `files[].url` is what
locates a file. The `*.dedi.json` name and the `/dedi/` directory are conventions; nothing in
verification or discovery consults them.
